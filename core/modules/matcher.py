...
def get_matching_data(candidate_id , job_id):
    try:
        ...
        cursor.execute("SELECT resume_text FROM candidates WHERE id = %s" , (candidate_id,))
...
MASTER_SKILLS = {
    ...
    'swift', 'kotlin', 'go', 'rust', 'r', 'bash', 'perl', 'scala', 'dart', 'matlab',
    
    # Web & UI (Frontend)
    'html', 'css', 'react', 'angular', 'vue', 'next.js', 'bootstrap', 'tailwind', 
    'jquery', 'sass', 'figma',
    
    # Backend Frameworks
    'django', 'flask', 'fastapi', 'spring', 'express', 'node.js', 'ruby on rails', 
    'asp.net', 'laravel', 'graphql', 'rest', 'api',
    
    # Cloud & DevOps
    'aws', 'azure', 'gcp', 'docker', 'kubernetes', 'jenkins', 'git', 'github', 
    'gitlab', 'terraform', 'ansible', 'linux', 'unix', 'ci/cd', 'microservices',
    
    # Databases
    'sql', 'nosql', 'mysql', 'postgresql', 'sqlite', 'mongodb', 'redis', 
    'cassandra', 'oracle', 'mariadb', 'elasticsearch', 'dynamodb', 'firebase',
    
    # Data Science, AI & Machine Learning
    'ai', 'ml', 'nlp', 'pandas', 'numpy', 'tensorflow', 'keras', 'pytorch', 
    'scikit-learn', 'matplotlib', 'seaborn', 'opencv', 'machine learning', 
    'deep learning', 'artificial intelligence', 'data science', 'big data', 
    'spark', 'hadoop', 'kafka', 'powerbi', 'tableau', 'neural networks',
    
    # Core Computer Science Concepts
    'dsa', 'oop', 'cybersecurity', 'agile', 'scrum', 'sdlc', 'json', 'xml'
}

def clean_for_nlp(text):

    stop_words = {'i', 'am', 'a', 'and', 'for', 'the', 'to', 'with', 'in', 'of', 'is', 'looking', 'we', 'are'}
    text = re.sub(r'[^a-z\s]', ' ', text.lower())
    words = text.split()

    unique_words = {word for word in words if word in MASTER_SKILLS and word not in stop_words}
...
def analyze_student_gap(student_id, target_job_id):
    ...
    if gap:
        ...
        print(f"🚀 YOUR STUDY LIST  : None! You have all the required skills!")
    print(f"=========================================\n")
